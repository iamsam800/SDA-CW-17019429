...
Metronome::Metronome()
{
    ...
    File MetronomeClick{ File::getSpecialLocation(File::SpecialLocationType::userDesktopDirectory) }; /** Make sure this is a file locaiton within folder */
    auto myMetronome = MetronomeClick.findChildFiles(File::TypesOfFileToFind::findFiles,true,  "MetronomeSoundSDA.wav"); /** Tick is a little queiet, I need to remember to increase gain on reaper or something*/
...
{
    //metSampleRate = DeviceSampleRate.getSampleRate();
    ...
    HighResolutionTimer::startTimer(60.0); /**Abstract method */
