...
/**Setting filter type to low pass, move into button press use cases or if statement */
  ...
  oscillator2 = &sin2;
   ...
   //lpf = new IIRFilter (IIRCoefficients::makeLowPass(sampleRate, sampleRate / 2, 1.0)); /** Setting bandwidth of synth to half samlpe rate */
...
void Audio::updateMidiMessage()
{
    ...
    if (m_Midi.isNoteOn)
    {
        ...
        lfo->setFrequency(currentFrequency);
    }
    if (!m_Midi.isNoteOn){ adsr.noteOff();}
...
void Audio::audioDeviceAboutToStart (AudioIODevice* device)
{
   ...
   // spec.sampleRate = sampleRate; /** Setting sample rate */
    
    Nyquist.setCoefficients(IIRCoefficients::makeLowPass(sampleRate, sampleRate / 2));
    Nyquist2.setCoefficients(IIRCoefficients::makeLowPass(sampleRate, sampleRate / 2));    //lpf = new IIRFilter (IIRCoefficients::makeLowPass(sampleRate, 1000, 1.0));
}
