...
myButtons::myButtons()
{
   ...
   hello.setSliderStyle(Slider::Rotary);
...
myButtons::myButtons(std::string name)
{
   /**Set Button text */
...
void myButtons::resized()
{
    ...
    float MetBPMxPosition = (MetGainSliderxPosition + MetGainSliderWidth) + 10;
     hello.setBounds(100, 100, getWidth(), getHeight());
...
void myButtons::setState(bool state)
{
    /** Set button state*/
    thisButton.setToggleState(state, dontSendNotification);
    
}
void myButtons::buttonClicked (Button* Button)
{
...
void myButtons::getmetButtonState()
{
    /** Getting button state then returning value of button state */
    std::atomic<int> met = metbutval;
    
 audioPtr->setMetButtonState(met);
    
   
}
void myButtons::getAudioReference(Audio &a)
{
    audioPtr = &a;
}

void myButtons::sliderValueChanged(Slider *slider)
{
